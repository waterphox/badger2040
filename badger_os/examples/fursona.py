#generate the fursona of your dreams (originally by softhoof)
...
altlabels = ("MENU", "SAVE", "SPEED")
alt2labels = ("MENU", "o3o", "CREDIT")
btnlabelsize = 0.6
...
#Renders the initial screen before any stuff hapset_pens, like a button press
def render(menuonly=0):
    if menuonly == 1:
        draw_menu()
        display.partial_update(0, 112, WIDTH, 16)
    else:
        global currentFurre
        ...
        #display.text("MEM FREE", memtextpos + 3, 22, 0.35)
        display.set_pen(15)
        display.text("generate the fursona of your dreams", 3, 8, scale=0.4)
        draw_menu()
        #display.line(memtextpos - 2, 0, memtextpos - 2, 16)
        display.set_pen(15)
        display.set_thickness(1)
...
def draw_menu():
    ...
    if menupage == 1:
        for x in range(0, 3):
            display.text(btnlabels[x], centers[x] - int(display.measure_text(btnlabels[x], btnlabelsize) / 2), 122, scale=btnlabelsize)
    elif menupage == 2:
        for x in range(0, 3):
            display.text(altlabels[x], centers[x] - int(display.measure_text(altlabels[x], btnlabelsize) / 2), 122, scale=btnlabelsize)
    elif menupage == 3:
        for x in range(0, 3):
            display.text(alt2labels[x], centers[x] - int(display.measure_text(alt2labels[x], btnlabelsize) / 2), 122, scale=btnlabelsize)
...
def button(pin):
    global menupage, font_size, inverted, currentFurre
    badger.led(255)
    if pin == button_a: #This button switches between menu options, and ALWAYS says Menu
        if menupage == 1:  #MENU, RE-DRAW, GENERATE!
            menupage = 2
        elif menupage == 2: #MENU, SAVE, SPEED
            menupage = 3
        else:               #MENU, o3o, CREDIT
            menupage = 1
        render(1)
    if pin == button_b:
        if menupage == 1:  #RE-DRAW
            render()
        elif menupage == 2: #SAVE
            pass
        else:               #o3o
            ...
            display.update()
    if pin == button_c:
        if menupage == 1:   #GENERATE!
            generateFursona()
            render()
        elif menupage == 2: #SPEED
            pass
        else:               #CREDIT
